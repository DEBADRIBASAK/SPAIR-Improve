...
class ImgEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.z_what_net = nn.Conv2d(64, z_what * 2, 1)
    ...
    def forward(self, x, tau):
        ...
        z_what_mean,z_what_std = self.z_what_net(cat_enc).chunk(2,1)
        q_z_what = Normal(z_what_mean,z_what_std)
        z_what = q_z_what.rsample()

        return z_where, z_pres, z_depth, q_z_where, \
               q_z_depth, z_pres_logits, z_pres_y, z_what, q_z_what
...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc_what = nn.Conv2d(128, z_what_dim * 2,1)
    ...
    def forward(self, x):
        ...
        z_what_mean, z_what_std = self.enc_what(x).chunk(2, 1)
...
class GlimpseDec(nn.Module):
    ...
    def __init__(self):
        ...
        self.dec_o = nn.Conv2d(16, 1, 3, 1, 1)
...
class Spair(nn.Module):
    ...
    def forward(self, x, global_step, tau, eps=1e-15):
        ...
        #                           (4 * 4 * bs, 1, glimpse_size, glimpse_size), inverse=False)

        # (4 * 4 * bs, dim)
        z_what, q_z_what = self.z_what_net(x)
        z_what = z_what.permute(0,2,3,1).view(-1,z_what_dim)

        # (4 * 4 * bs, N_CHANNELS, glimpse_size, glimpse_size)
