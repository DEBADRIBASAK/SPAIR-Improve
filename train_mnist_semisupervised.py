...
def main():
    ...
    parser.add_argument('--dataset_path', default='./training_skl.pt',type=str,help='path to the stored dataset')
    ...
    for epoch in range(int(args.start_epoch), args.epochs):
        ...
        for batch_idx, sample in enumerate(train_loader):
            ...
            recon_x, log_like, kl_z_what, kl_z_where, kl_z_pres, kl_z_depth,classifier_loss, log = \
            ...
            total_loss = - (log_like - kl_z_what - kl_z_where - kl_z_pres - kl_z_depth - classifier_loss)# - kl_bg_what)
...
#                    'bg': log['bg'].view(-1, 3, img_h, img_w),
                ...
                print_spair_clevr(global_step, epoch, local_count, count_inter,
                                  ...
                                  kl_z_where, kl_z_pres, kl_z_depth, classifier_loss)
                end_time = time.time()
                ...
                bbox = visualize1(imgs[:num_img_summary].cpu(), log['z_pres'][:num_img_summary].cpu().detach(),
                                 log['z_where_scale'][:num_img_summary].cpu().detach(),
                                 log['z_where_shift'][:num_img_summary].cpu().detach(),log['pred'].cpu().detach())
                boxes = torch.ones((1,N_CHANNELS,32,32));
                boxes = ((torch.stack((boxes,)*4*4*bs,dim=1).view(-1,N_CHANNELS,32,32)*log['z_pres'].view(-1,1,1,1))>.5).float()
                predicted_boxes = spatial_transform(boxes,log['z_where'].cpu().detach(),(4*4*bs,N_CHANNELS,img_h,img_w),inverse=True)
                predicted_boxes = (predicted_boxes.view(-1,4*4,img_h,img_w).sum(dim=1).unsqueeze(1)>0).float()
                intersection = predicted_boxes*gt
