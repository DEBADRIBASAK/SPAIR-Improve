...
class ZWhatEnc(nn.Module):
    ...
    def forward(self, x):
        ...
        z_what_size = torch.zeros(z_what_mean.shape)
        z_what_mean = z_what_mean.view(-1,N_OBJECTS,N_TOTAL//N_OBJECTS,z_what_dim).mean(-2)
        z_what_std = (z_what_std.view(-1,N_OBJECTS,N_TOTAL//N_OBJECTS,z_what_dim)**2).mean(-2)**0.5
        z_what_std = F.softplus(z_what_std)
        q_z_what = Normal(z_what_mean, z_what_std)
        indices = torch.arange(N_OBJECTS)*(N_OCCURRENCES*N_OCCURRENCES)
        for i in range(N_TOTAL//N_OBJECTS):
            z_what[:,indices] = q_z_what.rsample()
            indices+=1
...
class Spair(nn.Module):
    ...
    def forward(self, x, global_step, tau, eps=1e-15):
        ...
        #x_att = x_att.view(bs,N_OBJECTS,-1,N_CHANNELS,glimpse_size,glimpse_size).mean(2)
        #x_att = x_att.view(-1,N_CHANNELS,glimpse_size,glimpse_size)
        # (N_TOTAL*bs,dim)
