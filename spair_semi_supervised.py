...
class ZWhatEnc(nn.Module):
    ...
    def __init__(self):
        ...
        self.classifier = nn.Sequential(nn.Linear(z_what_dim,64),nn.ReLU(),nn.Linear(64,32),nn.ReLU(),nn.Linear(32,10),nn.LogSoftmax(dim=1))
    ...
    def forward(self, x,target):
        ...
        preds = self.classifier(z_what)

        l = self.loss(preds,target)
        #preds = torch.argmax(preds,dim=1)


        return z_what, q_z_what, preds, l
...
class Spair(nn.Module):
    ...
    def __init__(self,saved_model="./mnist_model.pt", sigma=0.3):
        ...
        self.model = nn.Sequential(nn.Conv2d(1,4,4,2,1),nn.ReLU(),\
    ...
    def forward(self, x, global_step, tau, eps=1e-15):
        ...
        # (4 * 4 * bs, dim)
        target = torch.argmax(self.model((x_att-0.5)/0.5),dim=1)
        #target = torch.argmax(self.model(x_att),dim=1)
        z_what, q_z_what,preds,classifier_loss = self.z_what_net(x_att,target)
        #z_pres_int = (z_pres.view(-1)>=0.5).float()
        classifier_loss = (classifier_loss*z_pres.view(-1,1)).view(-1,16).sum(dim=1).mean()
        
        pred_args = torch.argmax(preds,dim=1)
        z_pres_int = z_pres.view(-1,1)>=0.5
        acc = ((target*z_pres_int)==(pred_args*z_pres_int)).view(-1,16).float()
        acc = acc.sum(dim=1).mean()
        ...
        sigma_root_2 = torch.tensor(1/(self.likelihood_sigma*math.sqrt(2*math.pi))).cuda()
